...
# The following is written by m-dango
class Bowling {
    my class Frame {
        has @.rolls is List;
    ...
    has Frame @!frames = |(Frame.new xx 9), Frame.new(:is-final);

    method roll ($pins) {
        X::Bowling::GameOver.new.throw if $.is-complete;

        @!frames.first(*.is-complete.not).add-roll($pins);
    }
